...
Nevada's 513-partition build contains 1,272,844 buildings with 73.03% usable height coverage.
Las Vegas, Reno, and Carson City each passed live NWS and controlled 38 C route validation through
managed Mapbox and the private HTTP Overture service. Live validation averaged 2,597 ms; controlled
heat averaged 997 ms. Four disjoint resumable workers covered the complete state plan without
fixture fallback, and the canonical audit confirmed all 513 partitions with zero invalid stores.
Two transient R2 read-back requests recovered through bounded retry. All 2,053 R2 objects passed
remote byte-count and SHA-256 verification before local data was pruned.

New Mexico's 529-partition build contains 1,400,698 buildings with 55.90% usable height coverage.
Albuquerque, Santa Fe, and Las Cruces each passed live NWS and controlled 38 C route validation
through managed Mapbox and the private HTTP Overture service. Live validation averaged 1,968 ms;
controlled heat averaged 1,162 ms. Four disjoint resumable workers covered the complete state plan
without fixture fallback, and the canonical audit confirmed all 529 partitions with zero invalid
stores. Transient official STAC, R2 upload, and R2 read-back requests recovered through bounded
...
Montana's 783-partition build contains 991,381 buildings with 61.55% usable height coverage. Helena,
Billings, and Missoula each passed live NWS and controlled 38 C route validation through managed
Mapbox and the private HTTP Overture service. Live validation averaged 1,791 ms; controlled heat
averaged 599 ms. Four disjoint resumable workers covered the complete state plan without fixture
fallback. A shared upstream interruption was recovered without repeating completed partitions, and
the canonical audit confirmed all 783 partitions with zero invalid stores. Transient R2 upload and

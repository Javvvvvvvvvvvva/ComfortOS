...
function main() {
  ...
  const selected = options.partitionId
    ? plan.partitions.filter((partition) => partition.id === options.partitionId)
    : plan.partitions.slice(0, maximum);
  ...
  for (const partition of selected) {
    ...
    if (options.resume === "true" && fs.existsSync(path.join(outputDir, "manifest.json"))) {
      skipped += 1;
      continue;
    }
    if (dryRun) {
